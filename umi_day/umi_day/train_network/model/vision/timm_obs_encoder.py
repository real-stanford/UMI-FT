"""Modified from original UMI diffusion policy implementation to only apply image augmentations during eval."""
    ...
    def __init__(self,
        ...
        for key, attr in obs_shape_meta.items():
            ...
            if type == 'rgb':
                ...
                image_shape = shape[1:]
        if eval_transforms is not None and not isinstance(eval_transforms[0], torch.nn.Module):
            assert eval_transforms[0].type == 'CenterCrop'
            ratio = eval_transforms[0].ratio
            eval_transforms = [
                torchvision.transforms.CenterCrop(size=int(image_shape[0] * ratio)),
                torchvision.transforms.Resize(size=image_shape[0], antialias=True)
            ] + eval_transforms[1:]
        transform = nn.Identity() if eval_transforms is None else torch.nn.Sequential(*eval_transforms)
    ...
    def forward(self, obs_dict):
        ...
        for key in self.rgb_keys:
            ...
            if not self.training:
                # only apply transforms during eval
