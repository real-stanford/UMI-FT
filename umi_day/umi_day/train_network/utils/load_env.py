...
def load_env_runner(cfg, output_dir):
    if "libero" in cfg.task.name:
        ...
        if cfg.task.env_runner.include_file_filters:
            hdf5_files = [x for x in hdf5_files if os.path.basename(x).replace('.hdf5', '') in cfg.task.env_runner.include_file_filters]
...
def env_rollout(cfg, env_runners, policy):
    ...
    if "libero" in cfg.task.name:
        ...
        if "train/mean_score" in runner_log:
