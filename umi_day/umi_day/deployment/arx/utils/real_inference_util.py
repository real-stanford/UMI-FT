...
def get_real_umi_obs_dict(
    ...
    language_encodings: Dict[str, np.ndarray] = None,
) -> Dict[str, np.ndarray]:
    obs_dict_np = dict()
    # process non-pose
    obs_shape_meta = shape_meta["obs"]
    ...
    for key, attr in obs_shape_meta.items():
        ...
        shape = attr.get("shape")
        horizon = attr.get("horizon")
        if is_language:
            obs_dict_np[key] = language_encodings[key][-horizon:]
        elif type == "rgb":
            this_imgs_in = env_obs[key]
            t, hi, wi, ci = this_imgs_in.shape
            co, ho, wo = shape
            ...
            if (ho != hi) or (wo != wi) or (this_imgs_in.dtype == np.uint8):
                ...
                if this_imgs_in.dtype == np.uint8:
                    out_imgs = out_imgs.astype(np.float32) / 255
            # THWC to TCHW
            obs_dict_np[key] = np.moveaxis(out_imgs, -1, 1)[-horizon:]
        elif type == "low_dim" and ("eef" not in key):
            this_data_in = env_obs[key]
            obs_dict_np[key] = this_data_in[-horizon:]
            # handle multi-robots
            ks = key.split("_")
