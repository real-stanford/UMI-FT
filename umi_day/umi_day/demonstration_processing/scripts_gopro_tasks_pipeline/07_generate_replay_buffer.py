# Modified from UMI to add additional labels to the replay buffer (specifically `episode_name` and entries for subtasks).
...
def main(input, output, out_res, out_fov, compression_level, 
    ...
    for ipath in input:
        ...
        for plan_episode in plan:
            ...
            episode_name = os.path.dirname(cameras[0]['video_path'])
            
            tasks = plan_episode['tasks']
            
            out_replay_buffer.add_episode(data=episode_data, tasks=tasks, compressors=None, episode_name=episode_name)
