"""
Matches script from original UMI repo except:
1) removing the part that cuts clips that have too many lost frames. This is because iPhone pipeline marks frames at the start and end of the clip as lost if there are not corresponding iPhone poses available.
2) add labels dataset plan
"""
...
def main(input, output, tcp_offset, tx_slam_tag,
    ...
    for demo_idx, demo_data in enumerate(demo_data_list):
        ...
        for s, is_valid in zip(segment_slices, segment_type):
            ...
            # process the labels
            labels_path = video_dir.joinpath('labels.json')
            with open(labels_path, 'r') as f:
                tasks = json.load(f)

            # the computed `start` and `end` may be different from the start/end the labels, so we need
            # to trim the labels to match the computed `start` and `end` bounds
            for task in tasks:
                # task looks like {"name": str, "start_idx": int, "end_idx": int, "labels": {"label1": np.ndarray, ...}}
                original_start_idx = task['start_idx']
                original_end_idx = task['end_idx']
                task['start_idx'] = max(original_start_idx, start)
                task['end_idx'] = min(original_end_idx, end)

                cut_x_start_items = task['start_idx'] - original_start_idx
                cut_x_end_items = original_end_idx - task['end_idx']

                cur_labels = task['labels']

                for label_name in cur_labels:
                    label_value = np.array(cur_labels[label_name], dtype=np.float32)
                    if cut_x_start_items != 0:
                        label_value = label_value[cut_x_start_items:]
                    if cut_x_end_items != 0:
                        label_value = label_value[:-cut_x_end_items]

                    assert len(label_value) == task['end_idx'] - task['start_idx']
                    cur_labels[label_name] = label_value
